...
class Model_penjualan extends CI_Model
{
    ...
    {
        $this->db->select('penjualan_detail_temp.kode_barang,nama_barang,berat_barang,(qty * berat_barang) as berat,kode_harga,harga,qty,(qty * harga) as total,id_user');
    ...
    function insertPenjualan($data)
    {
        ...
        if ($simpan) {
            ...
            if($error>0){
                $hapusdetailpenjualan = $this->db->delete('penjualan', array('no_faktur' => $data['no_faktur']));
    ...
    {
        $this->db->select('penjualan_detail.kode_barang,nama_barang,berat_barang,(qty * berat_barang) as berat,penjualan_detail.harga,qty,satuan');
    ...
    {
        $hariini = date("Y-m-d");
        if($this->session->userdata('kode_cabang') != "ADM") {
